...
def display_table():
    ...
    if log_entries:
        for entry in log_entries[-5:]:
            ...
            if len(entry) > TABLE_WIDTH - 7:  # Account for "║   • " and " ║"
        ...
        if "Fitri Points:" in response:
            try:
                ...
                if len(parts) >= 2:
                    points = int(parts[1].strip())
                    last_known_points = points
                    add_log_entry(f"Initial points: {points}")
                    connection_status = "Connected (Initial)"
                    print(f"✓ Initial points received: {points}")
            except ValueError:
...
def start_client():
    ...
    print("╔" + "═" * (TABLE_WIDTH - 2) + "╗")
    title = "FITRI POINTS MONITOR - STARTING"
    print("║" + center_text(title, TABLE_WIDTH - 2) + "║")
    print("╠" + "═" * (TABLE_WIDTH - 2) + "╣")
    ...
    if seconds_to_next_interval > 0:
        ...
        for i in range(seconds_to_next_interval, 0, -1):
            if i <= 3 or i % 5 == 0:  # Show only last 3 and every 5th second
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # Send request and get response using safe function
...
if __name__ == "__main__":
    try:
        ...
        print(f"Final points: {last_known_points}")
        print(f"Total tables: {update_counter}")
        print("=" * TABLE_WIDTH)
        sys.exit(0)