...
#define PORT 5002  // Different port
...
void update_database() {
    char query[256];
    sprintf(query, "UPDATE user_points SET points = points + 1, datetime_stamp = NOW() WHERE user = 'faris'");
    ...
    if (mysql_query(conn, query)) {
        ...
        printf("[%s] Faris's points updated\n", time_buffer);
    }
}
...
void handle_client(int client_socket) {
    ...
    if (bytes_received > 0) {
        ...
        if (strstr(buffer, "GET_POINTS") != NULL) {
            char query[256];
            sprintf(query, "SELECT points FROM user_points WHERE user = 'faris'");
            ...
            if (mysql_query(conn, query)) {
                ...
                if (result != NULL) {
                    ...
                    char response[256];
                    sprintf(response, "Faris Points: %s", row[0]);
                    send(client_socket, response, strlen(response), 0);
...
int main() {
    printf("C Server (Faris) starting on port %d\n", PORT);
